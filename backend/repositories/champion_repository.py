...
from models import Champion, AnalysisLog
...
class ChampionRepository(BaseRepository[Champion]):
    ...
    async def get_by_status(self, status: str) -> list[Champion]:
        ...
        result = await self.session.execute(
            select(Champion)
            .where(Champion.status == status)
            .order_by(Champion.created_at.desc())
        )
        return list(result.scalars().all())

    async def get_all_ordered(self, status: Optional[str] = None) -> list[Champion]:
    ...
    async def get_ready_champion(self, champion_id: int) -> Champion | None:
        ...
        result = await self.session.execute(
            select(Champion).where(
                ...
                Champion.patterns_json.isnot(None)
            )
        )
...
class AnalysisLogRepository(BaseRepository[AnalysisLog]):
    ...
    async def get_champion_logs(self, champion_id: int) -> list[AnalysisLog]:
        ...
        result = await self.session.execute(
            select(AnalysisLog)
            .where(AnalysisLog.champion_id == champion_id)
