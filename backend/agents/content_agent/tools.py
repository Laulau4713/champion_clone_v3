...
from models import Skill, Sector, Course, Quiz
...
class ContentTools:
    ...
    async def get_sector_by_slug(self, slug: str) -> Optional[Sector]:
    ...
    async def get_course_for_day(self, day: int) -> Optional[Course]:
        """Get the course for a given day."""
        return await self.db.scalar(
            select(Course).where(Course.day == day)
        )

    async def get_quiz_for_skill(self, skill_id: int) -> Optional[Quiz]:
        """Get the quiz for a skill."""
        return await self.db.scalar(
            select(Quiz).where(Quiz.skill_id == skill_id)
        )

    async def get_skills_for_level(self, level: str) -> list[Skill]:
        """Get all skills for a level."""
        result = await self.db.execute(
            select(Skill).where(Skill.level == level).order_by(Skill.order)
        )
    ...
    async def get_all_skills(self) -> list[Skill]:
        ...
        result = await self.db.execute(
            select(Skill).order_by(Skill.level, Skill.order)
        )
        return list(result.scalars().all())
    ...
    async def get_all_sectors(self) -> list[Sector]:
        """Get all sectors."""
        result = await self.db.execute(
            select(Sector).order_by(Sector.name)
        )
        return list(result.scalars().all())
    ...
    async def get_courses_for_level(self, level: str) -> list[Course]:
        ...
        result = await self.db.execute(
            select(Course).where(Course.level == level).order_by(Course.day)
        )
        return list(result.scalars().all())
