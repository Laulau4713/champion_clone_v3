...
from models import (
    User, UserAchievement, UserXP, UserProgress,
    UserSkillProgress, VoiceTrainingSession
)
...
async def get_user_xp(
    ...
    xp_data = await db.scalar(
        select(UserXP).where(UserXP.user_id == current_user.id)
    )
...
async def _get_user_stats(user_id: int, db: AsyncSession) -> dict:
    ...
    sessions_count = await db.scalar(
        select(func.count(VoiceTrainingSession.id)).where(
            ...
            VoiceTrainingSession.status == "completed"
        )
    ) or 0
    ...
    perfect_sessions = await db.scalar(
        select(func.count(VoiceTrainingSession.id)).where(
            VoiceTrainingSession.user_id == user_id,
            VoiceTrainingSession.current_gauge >= 100
        )
    ) or 0

    # Count expert conversions
    expert_conversions = await db.scalar(
        select(func.count(VoiceTrainingSession.id)).where(
            VoiceTrainingSession.user_id == user_id,
    ...
    sessions_result = await db.execute(
        select(VoiceTrainingSession).where(
            VoiceTrainingSession.user_id == user_id
        )
    )
