...
async def list_errors(
    ...
    errors, total = await activity_service.get_errors(
        resolved=resolved, error_type=error_type, limit=per_page, offset=(page - 1) * per_page
    )
...
async def get_error_detail(error_id: int, admin: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
...
async def resolve_error(
    error_id: int, body: ErrorResolveRequest, admin: User = Depends(require_admin), db: AsyncSession = Depends(get_db)
):
