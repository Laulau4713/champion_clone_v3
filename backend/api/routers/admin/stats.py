...
from models import (
    User, Champion, TrainingSession, AdminAlert,
    SubscriptionPlan
)
...
async def get_admin_stats(
    ...
    avg_score = await db.scalar(
        select(func.avg(TrainingSession.overall_score))
        .where(TrainingSession.overall_score.isnot(None))
    )
    ...
    for plan in SubscriptionPlan:
        count = await db.scalar(
            select(func.count(User.id)).where(User.subscription_plan == plan.value)
        )
