  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "border-success-500/50 bg-success-500/10 text-success-400",
        error: "border-error-500/50 bg-error-500/10 text-error-400",
        warning: "border-warning-500/50 bg-warning-500/10 text-warning-400",
        info: "border-secondary-500/50 bg-secondary-500/10 text-secondary-400",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-slate-600 bg-transparent px-3 text-sm font-medium transition-colors hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
...
const ToastIcon: React.FC<{ variant?: "default" | "success" | "error" | "warning" | "info" }> = ({
  ...
  switch (variant) {
    case "success":
      return <CheckCircle className={cn(iconClass, "text-success-400")} />;
    case "error":
      return <AlertCircle className={cn(iconClass, "text-error-400")} />;
    case "warning":
      return <AlertTriangle className={cn(iconClass, "text-warning-400")} />;
    case "info":
      return <Info className={cn(iconClass, "text-secondary-400")} />;
    default:
      return null;
  }
};
